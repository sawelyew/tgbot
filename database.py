...
async def get_welcome_text():
    async with async_session() as session:
        result = await session.execute(
            select(BotSettings.value).where(BotSettings.key == "welcome_text")
        )
        return result.scalar_one()

async def change_welcome_text(welcome_text):
    async with async_session() as session:
        await session.execute(
            update(BotSettings).where(BotSettings.key == "welcome_text").values(value=welcome_text)
        )
...
async def get_contacts():
    async with async_session() as session:
        result = await session.execute(
            select(BotSettings.value).where(BotSettings.key == "contacts")
        )
        return result.scalar_one()

async def change_contacts(contacts):
    async with async_session() as session:
        await session.execute(
            update(BotSettings).where(BotSettings.key=='contacts').values(value=contacts)
        )
        await session.commit()
